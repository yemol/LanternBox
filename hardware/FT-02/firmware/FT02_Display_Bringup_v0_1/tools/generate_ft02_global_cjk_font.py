#!/usr/bin/env python3
"""Generate FT-02's global embedded CJK bitmap font.
...
def generate(font_path: Path, face_index: int, size: int, header: Path, source: Path) -> None:
    ...
    with source.open("w", encoding="utf-8") as f:
        f.write('#include "FT02_GlobalCJKFontData.h"\n\n')
        f.write("static const FT02FontIndexRecord ft02_cjk_24r_idx[] PROGMEM = {\n")
        for cp, offset in records:
            f.write(f"    {{0x{cp:04X}, {offset}}},\n")
        f.write("};\n\n")
        f.write("static const uint8_t ft02_cjk_24r_bin[] PROGMEM = {\n")
        for start in range(0, len(data), 20):
            f.write("    " + ", ".join(f"0x{v:02X}" for v in data[start:start+20]) + ",\n")
        f.write("};\n\n")
        f.write(f"const uint32_t FT02_CJK_24R_GLYPH_COUNT = {len(records)};\n")
        f.write("const FT02FontPack ft02_cjk_24r = {\n")
        f.write("    ft02_cjk_24r_idx,\n")
        f.write(f"    {len(records)},\n")
        f.write("    ft02_cjk_24r_bin,\n")
        f.write(f"    {len(data)}\n")
        f.write("};\n")
...
def main() -> None:
    ...
    p.add_argument("--header", type=Path, default=Path("src/FT02_GlobalCJKFontData.h"))
    p.add_argument("--source", type=Path, default=Path("src/FT02_GlobalCJKFontData.cpp"))
    args = p.parse_args()
