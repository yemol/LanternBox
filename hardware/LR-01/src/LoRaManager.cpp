...
static const int8_t LORA_PROBE_POWER_DBM = 19;
...
static const uint32_t MESH_NATIVE_NODE_NUM = 0x4C423002UL; // !4c423002, LanternBox FT-02 / FT02
...
static bool buildUserProto(const uint8_t publicKey[32], uint8_t* out, size_t maxLen, size_t& outLen) {
  ...
  if (!appendStringField(out, outLen, maxLen, 1, String("!4c423002"))) return false;
  if (!appendStringField(out, outLen, maxLen, 2, String("LanternBox FT-02"))) return false;
  if (!appendStringField(out, outLen, maxLen, 3, String("FT02"))) return false;
...
static bool parseDataProto(const uint8_t* data,
  ...
  while (pos < len) {
    ...
    } else if (fieldNo == 6 && wireType == 5) {
      if (pos + 4 > len) return false;
      requestId = ((uint32_t)data[pos]) |
...
LoRaManager::LoRaManager()
  : radio(new Module(FtHardware::LORA_NSS_PIN, FtHardware::LORA_IRQ_PIN, FtHardware::LORA_RST_PIN, FtHardware::LORA_BUSY_PIN)),
    ...
    lastTxPacketIdValue(0),
    txRevisionCounter(0),
    lastDeliveryRequestIdValue(0),
    lastDeliveryFromValue(0),
    lastDeliveryAckValue(false),
    deliveryRevisionCounter(0),
    lastRxMillis(0),
...
bool LoRaManager::handleDecodedMeshData(const uint8_t* plain, size_t plainLen, bool pkiEncrypted) {
  ...
  if (!parseDataProto(plain, plainLen, portnum, appPayload, appLen, wantResponse, requestId)) {
  ...
  if (portnum == MESH_PORT_ROUTING_APP) {
    frameKindText = "MESH_ROUTING";
    bool ackOk = false;

    // Routing.error_reason is field 3, varint. NONE(0) means ACK.
    size_t rp = 0;
    while (rp < appLen) {
      uint32_t tag = 0;
      if (!readVarintRaw(appPayload, appLen, rp, tag)) break;
      uint32_t fn = tag >> 3;
      uint32_t wt = tag & 0x07;
      if (fn == 3 && wt == 0) {
        uint32_t reason = 0;
        if (readVarintRaw(appPayload, appLen, rp, reason)) {
          ackOk = (reason == 0);
        }
        break;
      }
      if (wt == 0) {
        uint32_t ignored = 0;
        if (!readVarintRaw(appPayload, appLen, rp, ignored)) break;
      } else if (wt == 2) {
        uint32_t l = 0;
        if (!readVarintRaw(appPayload, appLen, rp, l) || rp + l > appLen) break;
        rp += l;
      } else if (wt == 5) {
        if (rp + 4 > appLen) break;
        rp += 4;
      } else if (wt == 1) {
        if (rp + 8 > appLen) break;
        rp += 8;
      } else {
        break;
      }
    ...
    if (requestId != 0) {
      lastDeliveryRequestIdValue = requestId;
      lastDeliveryFromValue = meshFrom;
      lastDeliveryAckValue = ackOk;
      deliveryRevisionCounter++;
      Serial.print("MESH_DELIVERY_RX request_id=");
      Serial.print(formatHex32(requestId));
      Serial.print(" from=");
      Serial.print(formatHex32(meshFrom));
      Serial.print(" status=");
      Serial.println(ackOk ? "ACK" : "NAK");
    }
    payloadText = ackOk ? "ACK" : "ROUTING";
    return true;
  }
...
String LoRaManager::sanitizeText(const String& text, size_t maxLen) const {
  // Preserve UTF-8 bytes. Host-side validation rejects oversized messages
  // before this function is called, so this path should not cut a character.
  String out;
  out.reserve(min((size_t)text.length(), maxLen));
  for (size_t i = 0; i < (size_t)text.length() && (size_t)out.length() < maxLen; i++) {
    uint8_t b = (uint8_t)text[(int)i];
    if (b == '\r' || b == '\n' || b == '\t') b = ' ';
...
void LoRaManager::rememberNodeSeen(uint32_t node) {
  ...
  if (slot < 0) return;
  nodeLastSeenMs[slot] = millis();
  nodeLastRssi[slot] = rssi;
  nodeLastSnr[slot] = snr;
  uint8_t hopStart = (meshFlags >> 5) & 0x07;
  uint8_t hopLimit = meshFlags & 0x07;
  nodeLastHops[slot] = hopStart >= hopLimit ? (int8_t)(hopStart - hopLimit) : -1;
...
String LoRaManager::nodeDisplayName(uint32_t value) const {
  if (value == MESH_NATIVE_NODE_NUM) return "TF-02";
...
bool LoRaManager::getNetworkNode(size_t index, LoRaNodeView& out) const {
  ...
  for (size_t i = 0; i < NODE_NAME_MAX; i++) {
    ...
    if (found == index) {
      ...
      out.lastRssi = nodeLastRssi[i];
      out.lastSnr = nodeLastSnr[i];
      out.hops = nodeLastHops[i];
      return true;
    }
    found++;
  }
...
uint32_t LoRaManager::lastTxPacketId() const { return lastTxPacketIdValue; }
uint32_t LoRaManager::txRevision() const { return txRevisionCounter; }
uint32_t LoRaManager::deliveryRevision() const { return deliveryRevisionCounter; }
uint32_t LoRaManager::lastDeliveryRequestId() const { return lastDeliveryRequestIdValue; }
uint32_t LoRaManager::lastDeliveryFrom() const { return lastDeliveryFromValue; }
bool LoRaManager::lastDeliveryAck() const { return lastDeliveryAckValue; }
uint32_t LoRaManager::rxErrorCount() const { return rxFailCounter; }
