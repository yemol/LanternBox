...
2. 当前阶段主要可用资料是 ai_triggers.json、emergency_guides.json 和本地 Wiki。
3. 不要编造本地资料中没有的事实。
4. 如果本地资料不足，可以给出通用安全建议，但必须说明这是基于一般原则。
5. 回答要适合语音播报，句子不要太长。
6. 回答应控制在 600 字以内，优先给行动步骤。
7. 不要在回答正文最后重复列出本地触发场景、指南标题和 Wiki 标题，页面会单独展示这些资料。
...
页面下方会单独显示触发场景、应急指南来源和本地 Wiki 来源卡片。
如果某个本地资料对后续阅读特别关键，可以在正文中轻量提示一次，例如：详细步骤可查看下方《停水后的用水优先级》。
"""
...
不要在正文末尾重复列出参考资料，触发场景、应急指南来源和本地 Wiki 来源卡片会由页面单独显示。
如果某一条资料是关键依据，可以在相关建议后轻量提示“可查看下方《资料标题》”。
"""
