#!/usr/bin/env python3
"""Run Batch11-C Waste / Recycling retrieval field cases.
...
if not os.environ.get("LANTERNBOX_WASTE_RECYCLING_FIELD_TEST_VENV") and VENV_PYTHON.exists():
    os.environ["LANTERNBOX_WASTE_RECYCLING_FIELD_TEST_VENV"] = "1"
...
CASES_PATH = ROOT / "tests" / "fixtures" / "waste_recycling_field_cases.json"
RESULTS_PATH = ROOT / "docs" / "knowledge" / "batch11_c_waste_recycling_field_test_results.json"
REPORT_PATH = ROOT / "docs" / "knowledge" / "batch11_c_waste_recycling_field_test_report.md"
...
NEGATION_MARKERS = ["不要", "不可", "不能", "不应", "不得", "不再", "不继续", "禁止", "停止", "避免", "不允许", "不适用于"]
SAFE_CONTEXT_MARKERS = ["常见错误", "错误操作", "风险提示", "不要", "不可", "不能", "不应", "不得", "禁止", "停止", "避免", "不允许", "不适用于"]

WASTE_RECYCLING_GUIDES = {f"DG-{number:04d}" for number in range(887, 895)}
OFF_DOMAIN_GROUPS = {
    "repair": {"repair"},
    "tools": {"tools"},
    "shelter": {"shelter"},
...
def is_waste_recycling_guide(guide: EvidenceCandidate) -> bool:
    return guide.id in WASTE_RECYCLING_GUIDES


def cross_domain_labels(case: dict[str, Any], guides: list[EvidenceCandidate]) -> list[str]:
    if not guides:
        return ["no_guide_evidence"]
    if is_waste_recycling_guide(guides[0]):
        return []
    ...
    for label, domains in OFF_DOMAIN_GROUPS.items():
        if label in watched and top_domains & domains:
            labels.append(f"waste_recycling_vs_{label}")
...
def build_report(results: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        ...
        f"- Waste / Recycling 主 Guide 进入率（全部 cases）：{summary['waste_recycling_primary_rate']}",
        "",
        "安全指标：",
        "",
    ...
    conflict_keys = ["fire", "agriculture", "hygiene", "contamination", "manufacturing", "energy", "medical", "safety", "food", "records", "repair", "shelter"]
    ...
    if summary["cross_domain_counts"]:
        ...
        lines.append("")
        lines.append("未发现 strict case 非 Waste / Recycling Guide 抢主位。")

    lines.extend(["", "### 重点抢主位分析", ""])
    lines.append(
    ...
    lines.extend(["", "## 6. Waste / Recycling Evidence 稳定性", ""])
    lines.append(f"- Waste / Recycling Guide 作为 top evidence 的比例：{summary['waste_recycling_primary_rate']}。")
    ...
    if summary["strict_fail"] or summary["strict_partial"]:
        lines.append("需要进入 Batch11-D Waste / Recycling Retrieval Root Cause Review 后再判断。当前不直接新增 profile。")
        ...
        lines.append("暂不建议新增 profile；可将 Waste / Recycling v0.1 作为 stable candidate 继续观察。")

    lines.extend(["", "## 8. 是否进入 Waste / Recycling Retrieval Root Cause Review", ""])
    if summary["strict_fail"] or summary["strict_partial"] or summary["cross_domain_count"]:
        lines.append("建议进入 Batch11-D Waste / Recycling Retrieval Root Cause Review。原因：存在 strict partial/fail 或 observation cross-domain 信号，需要判断是 profile 缺口、selector/ranking 问题、Guide 设计问题还是合理跨域。")
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Batch11-C Waste / Recycling retrieval field cases.")
    ...
    waste_recycling_primary = [
        bool(case["guides_selected"]) and case["guides_selected"][0]["id"] in WASTE_RECYCLING_GUIDES
        for case in case_results
    ]
    summary = {
        ...
        "waste_recycling_primary_rate": pct(sum(waste_recycling_primary), len(waste_recycling_primary)),
