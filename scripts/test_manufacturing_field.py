#!/usr/bin/env python3
"""Run Batch9-D Manufacturing & Production Foundation retrieval field cases.
...
if not os.environ.get("LANTERNBOX_MANUFACTURING_FIELD_TEST_VENV") and VENV_PYTHON.exists():
    os.environ["LANTERNBOX_MANUFACTURING_FIELD_TEST_VENV"] = "1"
...
CASES_PATH = ROOT / "tests" / "fixtures" / "manufacturing_field_cases.json"
RESULTS_PATH = ROOT / "docs" / "knowledge" / "batch9_d_manufacturing_field_test_results.json"
REPORT_PATH = ROOT / "docs" / "knowledge" / "batch9_d_manufacturing_field_test_report.md"
...
MANUFACTURING_GUIDES = {f"DG-{number:04d}" for number in range(871, 879)}
OFF_DOMAIN_GROUPS = {
    "repair": {"repair"},
    "tools": {"tools"},
    "shelter": {"shelter"},
...
def is_manufacturing_guide(guide: EvidenceCandidate) -> bool:
    return guide.id in MANUFACTURING_GUIDES


def cross_domain_labels(case: dict[str, Any], guides: list[EvidenceCandidate]) -> list[str]:
    if not guides:
        return ["no_guide_evidence"]
    if is_manufacturing_guide(guides[0]):
        return []
    ...
    for label, domains in OFF_DOMAIN_GROUPS.items():
        if label in watched and top_domains & domains:
            labels.append(f"manufacturing_vs_{label}")
...
def build_report(results: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        ...
        f"- manufacturing 主 Guide 进入率（全部 cases）：{summary['manufacturing_primary_rate']}",
        "",
        "安全指标：",
        "",
    ...
    conflict_keys = ["repair", "tools", "shelter", "recycling", "medical", "safety", "fire", "agriculture", "team"]
    ...
    if summary["cross_domain_counts"]:
        ...
        lines.append("")
        lines.append("未发现 strict case 非 manufacturing Guide 抢主位。")
    ...
    lines.extend(["", "## 6. Manufacturing Evidence 稳定性", ""])
    lines.append(f"- Manufacturing Guide 作为 top evidence 的比例：{summary['manufacturing_primary_rate']}。")
    ...
    if summary["strict_fail"] or summary["strict_partial"]:
        lines.append("需要进入 Batch9-E Manufacturing Retrieval Root Cause Review 后再判断。当前不直接新增 profile。")
        ...
        lines.append("暂不建议新增 profile；可将 Manufacturing & Production Foundation v0.1 作为 stable candidate 继续观察。")

    lines.extend(["", "## 8. 是否进入 Manufacturing Retrieval Root Cause Review", ""])
    if summary["strict_fail"] or summary["strict_partial"] or summary["cross_domain_count"]:
        lines.append("建议进入 Batch9-E Manufacturing Retrieval Root Cause Review。原因：存在 strict partial/fail 或 observation cross-domain 信号，需要判断是 profile 缺口、selector/ranking 问题、Guide 设计问题还是合理跨域。")
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Batch9-D manufacturing retrieval field cases.")
    ...
    manufacturing_primary = [
        bool(case["guides_selected"]) and case["guides_selected"][0]["id"] in MANUFACTURING_GUIDES
        for case in case_results
    ]
    summary = {
        ...
        "manufacturing_primary_rate": pct(sum(manufacturing_primary), len(manufacturing_primary)),
